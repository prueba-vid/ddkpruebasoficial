...
// --- 4. BINDING DE EVENTOS TÁCTILES / RATÓN PARA BOTONES ---
...
const keyMap = {
    ...
    'ArrowLeft': 'left',
    'ArrowRight': 'right',
    'Space': window.ACTIONS.SALTAR,
    'KeyJ': window.ACTIONS.GOLPEAR,
    'KeyK': window.ACTIONS.BLOQUEAR,
    'ShiftLeft': window.ACTIONS.DASH,
    'ShiftRight': window.ACTIONS.DASH,
    'KeyQ': window.ACTIONS.ESPECIAL,
    'KeyE': window.ACTIONS.ULTI,
    'Digit1': window.ACTIONS.ACT1,
    'Digit2': window.ACTIONS.ACT2,
    'Digit3': window.ACTIONS.ACT3,
    'Digit4': window.ACTIONS.ACT4
};
...
window.addEventListener('keydown', e => {
    ...
    if (action !== undefined) {
        if (typeof action === 'string') {
            dpadKeysPressed[action] = true;
            updateDpadMovement();
        } else {
            window.setAction(action, true);
        }
    }
});
...
window.addEventListener('keyup', e => {
    ...
    if (action !== undefined) {
        if (typeof action === 'string') {
            dpadKeysPressed[action] = false;
            updateDpadMovement();
        } else {
            window.setAction(action, false);
        }
    }
});
