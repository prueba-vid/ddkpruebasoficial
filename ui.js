...
// --- 6. SISTEMA MEJORADO DE TECLADO PARA PC ---
function getActionFromKey(e) {
    const key = e.key.toLowerCase();
    const code = e.code;

    // Movimiento
    if (key === 'w' || key === 'arrowup') return { type: 'move', dir: 'up' };
    if (key === 's' || key === 'arrowdown') return { type: 'move', dir: 'down' };
    if (key === 'a' || key === 'arrowleft') return { type: 'move', dir: 'left' };
    if (key === 'd' || key === 'arrowright') return { type: 'move', dir: 'right' };

    // Acciones y habilidades
    if (key === ' ' || code === 'Space') return { type: 'bit', bit: window.ACTIONS.SALTAR };
    if (key === 'c' || key === 'j') return { type: 'bit', bit: window.ACTIONS.GOLPEAR };
    if (key === 'k' || key === 'x') return { type: 'bit', bit: window.ACTIONS.BLOQUEAR };
    if (key === 'shift' || code === 'ShiftLeft' || code === 'ShiftRight') return { type: 'bit', bit: window.ACTIONS.DASH };
    if (key === 'q') return { type: 'bit', bit: window.ACTIONS.ESPECIAL };
    if (key === 'e' || key === 'r') return { type: 'bit', bit: window.ACTIONS.ULTI };

    // Teclas numéricas para Habilidades 1, 2, 3, 4
    if (key === '1' || code === 'Digit1') return { type: 'bit', bit: window.ACTIONS.ACT1 };
    if (key === '2' || code === 'Digit2') return { type: 'bit', bit: window.ACTIONS.ACT2 };
    if (key === '3' || code === 'Digit3') return { type: 'bit', bit: window.ACTIONS.ACT3 };
    if (key === '4' || code === 'Digit4') return { type: 'bit', bit: window.ACTIONS.ACT4 };
