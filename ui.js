...
// --- 4. BINDING DE EVENTOS A BOTONES SIN CAMBIOS VISUALES AL TOCAR ---
...
// --- 5. EVENTOS TÁCTILES, RATÓN/TOUCHPAD, JOYSTICK Y CRUCETA D-PAD ---
window.touchState = {
    inputX: 0,
    inputY: 0,
    camTheta: 0,
...
// CONTROL DE CÁMARA PARA RATÓN Y TOUCHPAD
...
window.addEventListener('mousemove', e => {
    ...
    // Movimiento con Pointer Lock (pantalla bloqueada por click)
    if (document.pointerLockElement) {
        applyCameraRotation(e.movementX, e.movementY);
        return;
    }

    // Movimiento arrastrando ratón / touchpad sin bloqueo
...
// EVENTOS TÁCTILES PANTALLAS TÁCTILES
...
window.addEventListener('touchmove', e => {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
        const touch = e.changedTouches[i];
        if (!window.isDpadMode && touch.identifier === window.touchState.joystickTouchId && rect) {
            updateJoystick(touch, rect);
...
// --- 7. SISTEMA DE ALTERNANCIA D-PAD / JOYSTICK ---
...
// --- 8. SELECTOR DE PERSONAJE Y PANTALLA COMPLETA ---
