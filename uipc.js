...
// --- 5. LÓGICA DE TECLADO Y RATÓN (HUD PC) ---
const keysPressed = {};
...
function updatePCMovement() {
    ...
    if (keysPressed['w'] || keysPressed['W']) y += 1;  // Adelante
    if (keysPressed['s'] || keysPressed['S']) y -= 1;  // Atrás
    if (keysPressed['a'] || keysPressed['A']) x -= 1;  // Izquierda
    if (keysPressed['d'] || keysPressed['D']) x += 1;  // Derecha

    // Normalizar vector si se presionan dos teclas a la vez
    if (x !== 0 && y !== 0) {
        x *= 0.7071;
        y *= 0.7071;
    }
...
window.addEventListener('keydown', e => {
    if (e.repeat) return; // Evitar disparos continuos si se mantiene presionada
    const key = e.key.toLowerCase();

    // Ignorar controles del juego si se está escribiendo en el chat
    ...
    switch (key) {
        ...
        case 'p':
        case 'f12':
            e.preventDefault(); // Prevenir comportamiento por defecto de F12 del navegador
            toggleFullscreen();
            break;
        case 'o':
...
// --- 6. EVENTOS DE RATÓN (HIT Y BLOCK) ---
window.addEventListener('mousedown', e => {
    // Evitar que el clic en botones de interfaz dispare el ataque/bloqueo
    if (e.target.closest('#settingsModal, #charDropdown, #top-left-bar, input')) return;
...
// Desactivar menú contextual para que el clic derecho funcione como Bloqueo libremente
window.addEventListener('contextmenu', e => e.preventDefault());

// --- 7. MOVIMIENTO DE CÁMARA CON EL RATÓN ---
let isMouseDownForCam = false;
let lastMouseX = 0;
let lastMouseY = 0;

window.addEventListener('mousedown', e => {
    if (e.button === 0) {
        isMouseDownForCam = true;
...
window.addEventListener('mouseup', e => {
    if (e.button === 0) isMouseDownForCam = false;
});

window.addEventListener('mousemove', e => {
    if (!isMouseDownForCam) return;
...
// --- 8. INICIALIZACIÓN DE LA INTERFAZ Y SELECTOR DE PERSONAJES ---
