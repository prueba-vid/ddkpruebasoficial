...
function updatePCMovement() {
    ...
    // CORREGIDO: W incrementa Y (adelante) y S decrementa Y (atrás)
...
// Bloqueo del puntero para control estilo 3D PC
...
window.addEventListener('mousemove', e => {
    ...
    // Captura movimiento tanto con PointerLock como con movimiento relativo standard
    const movementX = e.movementX || 0;
    const movementY = e.movementY || 0;

    if (document.pointerLockElement === document.body || movementX !== 0 || movementY !== 0) {
        window.touchState.camTheta -= movementX * 0.0025 * sens;
        window.touchState.camPhi = Math.max(0.45, Math.min(Math.PI / 1.9, window.touchState.camPhi - movementY * 0.002 * sens));
    }
});
