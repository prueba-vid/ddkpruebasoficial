...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (settingsBtn && settingsModal) {
        settingsBtn.addEventListener('click', () => {
            ...
            refreshNetworkUI(); // Actualizar estado al abrir
        });
    }
    ...
    if (shadowPlusToggle) {
        ...
        shadowPlusToggle.addEventListener('change', e => {
            ...
            if (isChecked && shadowToggle) {
                ...
                if (typeof window.applyShadowSettings === 'function') window.applyShadowSettings(false);
            }
    ...
    function applyElementScale(el, scale) {
        ...
        el.style.transform = `scale(var(--hud-scale, 1))`;
    }
    ...
    // Lógica de drag (simplificada para no alargar demasiado)
    draggables.forEach(el => {
    ...
    function saveElementHudState(el) {
        localStorage.setItem(`hud_pos_${el.id}`, JSON.stringify({ 
            left: el.style.left, top: el.style.top, scale: el.dataset.scale, opacity: el.dataset.opacity
        }));
    }
        ...
        draggableIds.forEach(id => {
            ...
            if (saved) {
                try {
                    ...
                    if (el) { 
                        el.style.position = 'fixed'; el.style.left = left; el.style.top = top;
                        if (scale) applyElementScale(el, parseFloat(scale));
                        if (opacity !== undefined) applyElementOpacity(el, parseFloat(opacity));
                    }
                } catch(e) {}
            }
        });
...
window.checkDevHitboxOption = function() {
    if (window.playerUsername === "VidMC3") {
        const settingsModal = document.getElementById('settingsModal');
        if (settingsModal && !document.getElementById('hitboxToggleContainer')) {
            const container = document.createElement('div');
            container.id = 'hitboxToggleContainer';
            container.style.cssText = "margin-top:10px; padding:10px; border-top:1px solid #555; color:#ffd700; font-weight:bold;";
            container.innerHTML = `<span>Ver Hitboxes (DEV)</span> <input type="checkbox" id="hitboxToggle" ${window.GameConfig.showHitboxes ? 'checked' : ''}>`;
            ...
            document.getElementById('hitboxToggle').addEventListener('change', e => {
                ...
                if (typeof window.updateHitboxVisibility === 'function') window.updateHitboxVisibility();
            });
        }
    }
};
