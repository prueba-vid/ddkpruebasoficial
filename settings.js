...
document.addEventListener('DOMContentLoaded', () => {
            ...
            domeSlider.addEventListener('input', e => {
                ...
                if (window.DomeManager) {
                    ...
                    if (domeInfo && domeInfo.requiresPassword) {
                        ...
                        if (!success) {
                            // Revertir slider al valor que tenía
    ...
    // --- CONTROL DE SHADERS EN CONFIGURACIÓN ---
    if (settingsModal) {
    ...
    draggables.forEach(el => {
        el.addEventListener('pointerdown', e => {
            ...
            selectedElement = el;
            selectedElement.style.outline = '2px solid #ffd700';
        ...
        draggableIds.forEach(id => {
            ...
            if (savedData) {
                try {
                    ...
                    if (data.scale) {
                        applyElementScale(el, parseFloat(data.scale));
                    }
                    if (data.opacity) {
                        applyElementOpacity(el, parseFloat(data.opacity));
                    }
                } catch (err) {
