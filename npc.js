...
// Hace que la barra mire directamente a la cámara/jugador (Billboard)
...
// Restringe una posición dentro de los bordes del mapa
...
// Garantiza que el NPC nunca sobrepase los límites del mapa (se ejecuta cada frame)
...
function applyKnockback(directionVector, totalDistance = 0.5, durationMs = 100) {
    if (!dummyMesh) return;
...
function receiveDamage(amount, attackerPosition, isPhysicalHit = true) {
    if (!dummyMesh) return;

    dummyHealth = Math.max(0, dummyHealth - amount);
    
    // Reinicia la cuenta de 15 segundos para la regeneración
    lastDamageTime = performance.now();
...
// Regeneración continua si han transcurrido 15 segundos sin daño
...
// Colisión física bidireccional entre el Jugador y el NPC
function checkDummyCollision(playerGroup) {
    if (!dummyMesh || !playerGroup) return;

    const playerRadius = 0.4;
    const dummyRadius = 0.55;
