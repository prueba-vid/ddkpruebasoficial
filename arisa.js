...
// Estado del escudo de Arisa
let arisaShieldHits = 0;
let arisaShieldMesh = null;
let arisaShieldTimer = null;
...
function resetArisaState(scene, playerGroup) {
    ...
    if (arisaShieldTimer) clearTimeout(arisaShieldTimer);
    if (arisaShieldMesh) disposeMesh(arisaShieldMesh, playerGroup || scene);
    if (activeAnvilPreview) disposeMesh(activeAnvilPreview, scene);

    arisaShieldHits = 0;
    arisaShieldMesh = null;
    arisaShieldTimer = null;
...
function useArisaSpecial(playerGroup, activeActions) {
    if (!canUseArisaSkill(activeActions)) return;
    if (arisaCooldowns.SPEC) return;

    triggerArisaCooldown('specBtn', 17, 'SPEC');

    if (arisaShieldTimer) clearTimeout(arisaShieldTimer);
    if (arisaShieldMesh) disposeMesh(arisaShieldMesh, playerGroup);

    const domeGeo = new THREE.SphereGeometry(1.2, 24, 24, 0, Math.PI * 2, 0, Math.PI / 1.8);
    arisaShieldMesh = new THREE.Mesh(domeGeo, ARISA_MAT);
    arisaShieldMesh.position.set(0, 0.4, 0);
    playerGroup.add(arisaShieldMesh);

    arisaShieldHits = 2;

    arisaShieldTimer = setTimeout(() => {
        if (arisaShieldMesh && arisaShieldMesh.parent) {
            disposeMesh(arisaShieldMesh, arisaShieldMesh.parent);
            arisaShieldMesh = null;
            arisaShieldHits = 0;
        }
    }, 5500);
...
function checkArisaShieldHit() {
    if (arisaShieldHits > 0 && arisaShieldMesh) {
        arisaShieldHits--;
        if (arisaShieldHits <= 0) {
            if (arisaShieldTimer) clearTimeout(arisaShieldTimer);
            if (arisaShieldMesh.parent) disposeMesh(arisaShieldMesh, arisaShieldMesh.parent);
            arisaShieldMesh = null;
        }
        return true;
    }
...
// --- 3. ACT2: SOGA (CORREGIDA LA ATRACCIÓN) ---
...
function useArisaAct2(playerGroup, scene, activeActions) {
    ...
    triggerArisaCooldown('act2', 10, 'ACT2');
    ...
    // Calcular dirección de lanzamiento del jugador
    ...
    function animateRope() {
        ...
        if (state === "THROW") {
            ...
            if (typeof dummyMesh !== 'undefined' && dummyMesh) {
                ...
                // Margen de detección del objetivo
            ...
            if (caughtTarget) {
                // Vector en 2D (XZ) directo desde el enemigo hacia Arisa
                const pullDir = new THREE.Vector3()
                    .subVectors(playerGroup.position, caughtTarget.position);
                pullDir.y = 0; // Evitar que cambie la altura Y del piso

                const distanceToPlayer = pullDir.length();

                if (distanceToPlayer > 1.2) {
                    pullDir.normalize();
                    // Mover al enemigo progresivamente cuadro a cuadro
                    caughtTarget.position.addScaledVector(pullDir, throwSpeed * 1.3);
                } else {
                    state = "END";
                }

                // Posicionar cabeza de soga en la cintura del enemigo capturado
...
function useArisaAct3(playerGroup, scene, activeActions) {
    ...
    function animateStabArm(now) {
        ...
        if (elapsed < 0.12) {
            ...
            // Detección de daño en el punto máximo de estocada
