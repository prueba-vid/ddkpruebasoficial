...
window.setupNetworkCallbacks = function() {
    ...
    // Fuerza a iniciar la partida en ambos clientes para renderizar los elementos
    window.isGameStarted = true;

    window.Network.enviarDatos({
        type: 'INIT_PLAYER',
        skin: localStorage.getItem('selectedSkin') || window.currentCharacter || 'Kai',
...
window.handleNetworkData = function(data) {
    ...
    switch (data.type) {
        ...
        case 'DAMAGE_NPC':
            // Recibir daño emitido por el otro jugador
            if (typeof window.receiveDamage === 'function') {
...
// Usa esta función en tu script de combate cada vez que Golpees al NPC
window.atacarNPCYNotificar = function(montoDano, posicionAtacante, esFinisher) {
    // 1. Infligir daño en tu pantalla
    if (typeof window.receiveDamage === 'function') {
        window.receiveDamage(montoDano, posicionAtacante, false, esFinisher);
    }

    // 2. Notificar al rival para que también reciba el golpe
