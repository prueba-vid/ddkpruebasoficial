...
// Se ejecuta al abrirse la conexión WebRTC en ambos lados
window.setupNetworkCallbacks = function() {
    if (!window.Network || !window.Network.conexion) return;

    // Notificar la entrada del jugador local
...
window.handleNetworkData = function(data) {
    ...
    switch (data.type) {
        case 'INIT_PLAYER':
            ...
            // Responder con los datos propios para confirmar el saludo mutuo
