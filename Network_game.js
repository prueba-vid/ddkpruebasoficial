...
window.setupNetworkCallbacks = function() {
    ...
    // Enviar datos del personaje local al conectar
...
window.handleNetworkData = function(data) {
    ...
    switch (data.type) {
        case 'INIT_PLAYER':
            ...
            // Responder confirmación para que el host también cree al cliente
            window.Network.enviarDatos({
                type: 'SYNC_RESPONSE',
                skin: window.currentCharacter || 'Kai',
                name: window.playerUsername || 'Jugador'
        ...
        case 'DAMAGE_NPC':
            ...
            } else if (typeof receiveDamage === 'function') {
...
// Función ejecutada constantemente en tu bucle principal (requestAnimationFrame)
...
// Función helper para invocar cuando le haces daño a un NPC en tu pantalla local
window.syncNpcDamage = function(amount, attackerPos, isFinisher) {
    if (window.Network && window.Network.conexion && window.Network.conexion.open) {
        window.Network.enviarDatos({
            type: 'DAMAGE_NPC',
            amount: amount,
            attackerPos: attackerPos,
            isFinisher: isFinisher || false
        });
    }
};
