...
window.handleNetworkData = function(data) {
    ...
    switch (data.type) {
        ...
        case 'DAMAGE_NPC':
            // Aplicar el daño en la escena local cuando el otro jugador ataca al NPC
            if (typeof window.aplicarDanoNPCLocal === 'function') {
                window.aplicarDanoNPCLocal(data.amount, data.attackerPos, data.isFinisher);
            } else if (typeof window.receiveDamage === 'function') {
                window.receiveDamage(data.amount, data.attackerPos, false, data.isFinisher);
            }
            break;
        ...
        case 'PLAYER_ATTACK_ANIM':
            if (window.RemotePlayers['rival'] && typeof window.triggerHitAnimation === 'function') {
                window.triggerHitAnimation(window.RemotePlayers['rival']);
            }
            break;
    }
};
...
function createRemotePlayer(skinName, username) {
    ...
    if (typeof NameTagSystem !== 'undefined' && NameTagSystem.attachToPlayer) {
...
// Llama a esta función dentro de tu bucle principal de renderizado (requestAnimationFrame)
...
// Usa esta función en tu código cuando Golpees al NPC
window.atacarNPCYNotificar = function(montoDano, posicionAtacante, esFinisher) {
    // 1. Infligir daño en tu pantalla local
    if (typeof window.aplicarDanoNPCLocal === 'function') {
        window.aplicarDanoNPCLocal(montoDano, posicionAtacante, esFinisher);
    }

    // 2. Transmitir el daño al rival por la red
    if (window.Network && window.Network.conexion && window.Network.conexion.open) {
        window.Network.enviarDatos({
            type: 'DAMAGE_NPC',
            amount: montoDano,
            attackerPos: posicionAtacante,
            isFinisher: esFinisher || false
        });
    }
};
