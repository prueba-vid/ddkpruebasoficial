...
function animate() {
    ...
    // SISTEMA DE VUELO (FLY) Y MOVIMIENTO
    if (window.GameConfig?.flyEnabled && window.playerUsername === "VidMC3") {
        ...
        if (isMoving && window.touchState) {
            ...
            moveVector.addScaledVector(flyDir, -inputY);
            moveVector.addScaledVector(flyRight, inputX);

            if (moveVector.lengthSq() > 0) {
                moveVector.normalize();
                playerGroup.position.addScaledVector(moveVector, window.moveSpeed * delta);
